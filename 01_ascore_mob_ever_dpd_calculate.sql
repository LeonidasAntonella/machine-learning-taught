...
DROP TABLE IF EXISTS risk_data.ascore_ever_dpd_req_id FORCE
;

CREATE TABLE IF NOT EXISTS risk_data.ascore_ever_dpd_req_id AS
SELECT
    req_id_pre_credit AS req_id,
    account_id AS busi_account_id,
    req_dt_pre_credit AS req_dt,
    DATE_FORMAT(date_trunc ('week', req_dt_pre_credit), '%Y-%m-%d') AS req_dt_week,
    DATE_FORMAT(req_dt_pre_credit, '%Y-%m') AS req_dt_month,
    scene_pre_credit as scene,
    req_at_pre_credit AS create_at,
    order_id,
...
select req_dt_month, avg(mob1_7) from risk_data.ascore_ever_dpd_req_id 
...
from (
    ...
    where dt in ('2025-12-01', '2025-12-15', '2026-01-01', '2026-01-15', '2026-02-01', '2026-02-15')
